...
int main() {
   ...
   float round_input[4] = {-4.5f, -1.5f, 1.5f, 4.5f};
   float round_output[20];
   cl_mem round_input_buffer, round_output_buffer;
   ...
   program = build_program(context, device, "simple_rounding.cl");
   kernel = clCreateKernel(program, "rounding", &err);
   if(err < 0) {
   ...
   clSetKernelArg(kernel, 0, sizeof(cl_mem), &round_input_buffer);
   clSetKernelArg(kernel, 1, sizeof(cl_mem), &round_output_buffer);   
