...
//
int valuesOK(cl_int* from, cl_int* to, size_t length) {
#ifdef DEBUG
...
int main(int argc, char** argv) {
   ...
   cl_int16* ud_in = (cl_int16*) malloc( sizeof(cl_int16) * DATA_SIZE); // input to device
   cl_int16* ud_out = (cl_int16*) malloc( sizeof(cl_int16) * DATA_SIZE); // output from device
   for( int i = 0; i < DATA_SIZE; ++i) {
       ud_in[i] = (cl_int16) { (int)i,(int)i,(int)i,(int)i, (int)i,(int)i,(int)i,(int)i, (int)i,(int)i,(int)i,(int)i, (int)i,(int)i,(int)i,(int)i };
   }
   ...
   for(cl_uint i = 0; i < numOfPlatforms; i++ ) {
	    ...
	    cl_mem inobj = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 
	                                  sizeof(cl_int16) * DATA_SIZE, ud_in, &error);
	    if(error != CL_SUCCESS) {
        ...
        for(int i = 0; i < numOfDevices; ++i, ++offset ) {
	        ...
	        const char *file_names[] = {"vector_store.cl"}; 
	        ...
	        for(cl_uint j = 0; j < numOfKernels; j++) {
	            ...
	            cl_mem outobj = clCreateBuffer(context, CL_MEM_WRITE_ONLY, 
	                                           sizeof(cl_int16) * DATA_SIZE, 0, &error);
	            if (error != CL_SUCCESS) { 
                ...
                size_t threadsPerGroup[1] = {1};
                size_t numOfGroups[1] = { DATA_SIZE / (64 * threadsPerGroup[0]) };
	            ...
	            error = clEnqueueReadBuffer(cQ, outobj,
	                                         ...
	                                         sizeof(cl_int16)*DATA_SIZE,          // how much to copy
	                                         ud_out, 0, NULL, NULL);
